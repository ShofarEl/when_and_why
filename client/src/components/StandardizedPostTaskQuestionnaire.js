...
const StandardizedPostTaskQuestionnaire = ({ condition, taskNumber, onComplete, isTransferTask = false }) => {
  ...
  const agencyQuestions = [
    ...
    "I felt pressure to follow AI suggestions", // reverse scored - N/A for no-AI tasks
    "I felt the AI respected my autonomy", // N/A for no-AI tasks
  ...
  // Adjust questions based on condition
  const getRelevantAgencyQuestions = () => {
    if (isTransferTask || condition?.timing === 'none') {
      // For no-AI conditions, skip AI-specific questions
      return agencyQuestions.filter((_, index) => ![3, 4].includes(index));
    }
  ...
  const validateResponses = () => {
    ...
    // Only validate dependence for AI-assisted tasks
    if (!isTransferTask && condition?.timing !== 'none' && !responses.dependence) {
  ...
  const getConditionDescription = () => {
    if (isTransferTask) {
      return 'Independent problem-framing (No AI assistance)';
    }
    
    if (!condition) return 'Task completion';
  ...
  const relevantAgencyQuestions = getRelevantAgencyQuestions();
            ...
            <div className="flex items-center">
              ...
              <div>
                <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-white">
                  {isTransferTask ? 'Transfer Task' : `Task ${taskNumber}`} Feedback
                </h2>
                <p className="text-xs sm:text-sm md:text-base text-emerald-100">How was your experience?</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl md:rounded-2xl p-4 md:p-6 border border-blue-100">
              ...
              <div className="space-y-4 md:space-y-6">
                {relevantAgencyQuestions.map((question, index) => (
                  <div key={index} className="bg-white rounded-lg md:rounded-xl p-4 md:p-6 shadow-sm border border-gray-100">
                    <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-3 md:mb-4">
                      {question}
                      {(question.includes('AI') && (isTransferTask || condition?.timing === 'none')) && 
                        <span className="text-gray-400 text-xs ml-2">(N/A - No AI used)</span>
                      }
                    </label>
            ...
            {/* AI Dependence - Only for AI-assisted tasks */}
            {!isTransferTask && condition?.timing !== 'none' && (
            ...
            {/* Cognitive Load - Consistent 7-point scale */}
...
export default StandardizedPostTaskQuestionnaire;